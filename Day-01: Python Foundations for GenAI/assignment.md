...
**Test file:** Use the provided `../RAG assets/sample.txt` (8 sentences, 109 words). It deliberately uses `.`, `!` and `?` so your sentence splitter is exercised on all three.
...
3. **For a directory:**
   - Processes all `.txt` files in the directory (try `../RAG assets/documents/`, which holds 61 of them)
...
```bash
python document_analyzer.py "../RAG assets/sample.txt" --output json --chunk-size 200
python document_analyzer.py "../RAG assets/documents/" --output both
```
...
```python
stats = calculate_document_stats("../RAG assets/sample.txt")
# Output: {
#     'characters': 668,
#     'words': 109,
#     'sentences': 8,
#     'avg_words_per_sentence': 13.6,
#     'most_common_word': ('the', 13)
# }
```
...
=====================
Processing: ../RAG assets/sample.txt
✓ File processed successfully
  - Words: 109
  - Sentences: 8
  - Chunks: 4
  - Top word: 'the' (13 occurrences)
